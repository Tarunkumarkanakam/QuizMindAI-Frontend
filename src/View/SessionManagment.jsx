...
const SessionManagement = () => {
  ...
  const handlePageNumberInputChange = (value) => {
    let pageNumber = Number(value);
    if (isNaN(pageNumber) || pageNumber < 1) {
      pageNumber = 1;
    } else if (pageNumber > Math.ceil(filteredSessionData.length / sessionsPerPage)) {
      pageNumber = Math.ceil(filteredSessionData.length / sessionsPerPage);
    }
          ...
          {filteredSessionData.length > 0 ? (
            <div className="overflow-x-auto">
              ...
              <div className="flex justify-center mt-4">
                <div className="flex items-center space-x-2 px-4 py-2 bg-white border rounded-full shadow-md">
                  <button
                    onClick={prevPage}
                    className="px-4 py-2 text-indigo-500 hover:text-indigo-700 focus:outline-none"
                    disabled={currentPage === 1}
                  >
                    Previous
                  </button>
                  <div className="flex items-center">
                    <span className="px-2 text-gray-700">Page</span>
                    <input
                      type="number"
                      value={pageNumberInput}
                      onChange={(e) =>
                        handlePageNumberInputChange(e.target.value)
                      }
